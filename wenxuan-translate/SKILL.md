...
description: |
  专业中英翻译引擎（英文 → 中文为主）；当用户使用 /wenxuan-translate、提供英文链接/英文 Markdown 文档/英文粘贴文本、要求翻译或高质量中文译文时，启动双轨质量门：主 agent 边译边注入 humanlize 心法（位置/代价/手迹），完成后人味 sub-agent 复检，事实核查+溯源 sub-agent 并行校验（F1-F10 十大质量门），最终产出**纯中文 Markdown**（v1.6 起不再生成 PDF）。核心场景：H1 爆款选题（意译+中文地道）/ H2 行业认知（直译+术语锁定）/ H4 个人收藏（直译+极简）。
---
...
1. **人是目的，机器是手段**——专业翻译不是"AI 替我翻"，而是"AI 让我翻得更准"。所有质量门（F1-F10）服务于"忠实原文 + 中文地道"。
...
5. **默认输出纯中文**——主输出文件只用中文，文末附原文来源。不再产出中英对照模式。
...
| **F10** | **图片保留门** | 源文档所有图片 1:1 复制到译文（不下载/不丢/不加图说），译文图片数 ≥ 源图片数 | 14 张 X 推文截图，译文 0 张 → 全部丢失 |

### Step 6：输出（纯中文 Markdown）

**主 agent 输出**：

- `D:\path-to-wealth-freedom\内容\文章\翻译\译文-<原标题或日期>.md` —— 纯中文 Markdown（文末附原文来源）

**v1.6 变更**：不再生成 PDF。`scripts/md_to_pdf.py` 已删除。后续如需 PDF，自己用 Typst/Pandoc/macdown 等本地工具转换，或用浏览器打印。
...
<中文正文，按原文小节组织。代码块、列表、引用按原文结构保留，专有名词保留英文。>
...
> v1.6 起**不生成 PDF**。如需 PDF，用 Typst / Pandoc / Obsidian 导出等本地工具。
