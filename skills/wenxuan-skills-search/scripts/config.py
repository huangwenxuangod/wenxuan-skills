...
from env_utils import find_project_root, load_project_env, load_wenxuan_runtime_env
...
def load_dotenv(path: str = "") -> List[str]:
    ...
    if path:
        ...
        loaded = load_wenxuan_runtime_env(PROJECT_ROOT, cwd=Path.cwd(), skill_root=skill_root)
        ENV_LOADED = True
        return loaded
...
def env_safety_report() -> Dict[str, object]:
    ...
    protected_patterns = [".env", "*.env", ".env.local", "skills/wenxuan-skills-search/.env", "skills/wenxuan-skills-search/.env.local"]
