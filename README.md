...
| **`wenxuan-writer`** | 可用 | 按文轩的「踩坑大学生」口吻写公众号长文 | "写文章""帮我写""按我的风格写""AI 味太重" | 774 行 |
...
| **`wenxuan-translate`** | 可用 | 专业英文→中文翻译，humanlize 心法双注入 + 10 大质量门（含 F10 图片保留门） | `/wenxuan-translate` "翻译这个" "H1 翻译" | 262 行 |
...
7. 刷到一篇高质量英文推文/Newsletter/技术博客想翻译 → **`wenxuan-translate`** 纯中文 MD 译文（humanlize 心法双注入 + 10 大质量门 + 声音校准 4 维 + 文化处理 5 步）
...
└── wenxuan-translate/      # 专业英中翻译
    ├── SKILL.md
    ├── agents/openai.yaml
    ├── references/        # humanlize 心法、4 大翻译原则、10 大质量门、术语表
    └── scripts/   # v1.6+ 已删除 PDF 生成,只输出纯 MD
```
